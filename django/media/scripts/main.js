...
var map = void 0,
    ...
    MarkerClusterer = __webpack_require__(3);
...
GoogleMapsLoader.load(function (google) {
	...
	var resizeToImage = function resizeToImage() {
	...
	$.get('media/locations.kml', function (rawData) {
		//loop through placemarks tags
		$(rawData).find('Placemark').each(function (index, element) {
			//get coordinates and place name
			var url = $(element).find('name').text(),
			    desc = $(element).find('description').text();
			console.log($(element));
			var data = JSON.parse(desc);
		...
		$('[data-button="fullscreen"]').on('click', function (event) {
		...
		$('.gallery__image-caption .gallery__button').on('click', function (event) {
			changeImage($(event.currentTarget).data('button'));
		});

		//bind clicks on your navigation to scroll to a placemark
		$('.navigation li').on('click', function (event) {
			var panToPoint = new google.maps.LatLng(nav[$(event.currentTarget).index()].lng, nav[$(event.currentTarget).index()].lat);
			map.panTo(panToPoint);
		});

		$('.gallery__select').on('change', function (event) {
